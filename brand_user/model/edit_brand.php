...
if (empty($_POST['brandname'])) {
    ...
    echo json_encode($response);
} elseif(!empty($_POST['brandname'])  && !empty($_POST['address'])  && !empty($_POST['personname']) && !empty($_POST['phone']) && !empty($_POST['email']) && !empty($_POST['country']) && !empty($_POST['state']) && !empty($_POST['city'])&& !empty($reason)  && !empty($updateby)) {
...
$x=editBrand($id,$brandname,$outlets,$address,$pincode,$country,$state,$city,$personname,$designation,$phone,$email,$reason,$updateby);
//echo($x);exit;
if ($x==0) {
